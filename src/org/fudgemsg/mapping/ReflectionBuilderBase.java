...
/* package */class ReflectionBuilderBase<T> {
  ...
  private static void findMethods(final Class<?> clazz, final Map<String, Method> methods, final String prefix,
      final int paramLength, final Class<?> recurseLimit) {
   ...
   * 
   * @param clazz  the class to be serialized or deserialize by the builder
   * @param prefix  the method prefix, e.g. {@code get} or {@code set}
   * @param paramLength  the number of expected parameters
   * @param recurseLimit  the class to stop at when processing the superclasses
   */
  protected ReflectionBuilderBase(final Class<T> clazz, final String prefix, final int paramLength,
      final Class<?> recurseLimit) {
